...
class Store1 extends ChangeNotifier {
  ...
  Future<bool> getData() async {
    ...
    await firestore.collection("podcast").orderBy('date', descending: true).get().then((QuerySnapshot ds) {
      for (var doc in ds.docs) {
    ...
    for (int i=0; i<opp.length; i++) {
      final ref = FirebaseStorage.instance.ref().child('school_logo/${opp[i]}.png');
      var url = await ref.getDownloadURL();
      logoUrl.add(url);
    }
...
class Store2 extends ChangeNotifier {
  ...
  void fetchData(String url, int index) async {
    if (await webScraper
        .loadWebPage(url)) {
      title![index] = webScraper.getElement(
          'h1.entry-title', ['class']);
      category[index] = webScraper.getElement(
          'div.meta-category > a.category-style > span.label', ['class']);
      image[index] = webScraper.getElement(
          'div.entry-overlay > img', ['src', 'class']);
      read[index] = webScraper.getElement(
          'li.meta-reading-time', ['class']);
      date[index] = webScraper.getElement(
          'li.meta-date', ['class']);
    }
    notifyListeners();
  }
